...
### Todo
- 全ての単語を打てるようにする
  - タイピングライブラリのバグを、テストをしながら修正していく。用意した全ての単語がバグなく打てるようにする。
  - 漢字を含んでいるワードの対応を考える（読みがなを入れて、どこまで進めば漢字をどこまで進めるか分かるようにする必要がある）
    - 例えば漢字を含んでいるワードは、`display: 可能性, kana: [か, のう, せい]`みたいにするとか。
    - かなだけのワードも、`display: する, kana: [す, る]`と同じように扱える
