...
- [x] まず、上記のグラフを作成する
- [x] 逆向きに辺を張ったグラフを作成して、ゴールから各頂点への最短距離を求める
- [x] スタートからゴールまでの最短経路Rを計算して、保持しておく
  - この例の場合は、R = "kyuu"（実際は文字列で持つわけではないが、説明のため）
- [x] 正誤判定は、入力したキーに対応する辺がグラフにあるかを確認する
- [x] キーが正しいが最短路ではない場合は、その頂点からのゴールまでの最短経路をRに合流するまでたどって、Rをその経路で更新する。
  - 例えば、"ki"と打った場合は、そこから"xyu"とたどれば合流するので、R = "kixyu" + "u"に更新する
  - →とりあえず最短経路を全部再計算する方向で実装してもらった
