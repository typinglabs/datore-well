...
- [x]  `src/`ディレクトリは作らない方が良い？
  - なるべくフラットであって欲しいのと、プロジェクトルートにmoon.pkg.jsonがあるから、ここにmbtファイルを置く想定なのではないのか。
- [x] ファイルシステムに関するコードが`todo.mbt`にあるのは違和感があるので、切り出したい。`main.mbt`にファイルパスが定義されているのも、良くない。
  - Enterprise FizzBuzzっぽいが、`todo`パッケージと`永続化`パッケージを作るとよさそう。
- [x] CLIを実際にテストするコードが、実際のファイルシステムを使っているのを変えた方がいいかもしれない。 → これはtmpファイル使ってよしとしました
- [x] 上に関連して、CLIを実際にテストするコードが、前の結果に依存しているのを直した方がいいかもしれない（結合テストと考えたらそれでもいい？）
- [ ] テストコードを実行するときに、標準出力に出しているログがノイズになっている

```
todo_cli/
  task/ -- コアロジック
    task.mbt
  persistence/ -- ファイルシステムの読み書き
    persistence.mbt
  todo_cli.mbt
```
